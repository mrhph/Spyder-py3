...
def newDf():
    ...
    col = ['Region', 'Country', 'City/station', 'PM 2.5', 'PM2.5 Year']
    new_df[col] = df[col]
...
def sortPM():
    # PM2.5 降序
    df = pandas.read_csv(inputFile('.txt'), sep=' ', encoding="gbk")
    df.sort_values('PM 2.5', ascending=False, inplace=True)  # 在原数据集上排序
    print('#PM2.5降序排序完成！开始保存文件...')
    out_path = outputFile('.csv')
    df.to_csv(out_path, index=0, encoding="gbk")
    print('=>文件保存成功，{}'.format(out_path))
...
def disperse():
    ...
    dis = pandas.cut(df['PM 2.5'], category, labels=labels)  # 离散化
    nums = dis.value_counts()  # 统计离散数值的数量
    # 柱状图
    plt.bar(range(4), nums, color='blue')
    plt.xticks(range(4), labels)
    plt.xlabel('离散标签')
    plt.ylabel('数量')
    plt.title('PM2.5 离散化结果统计数值柱状图')
    # plt.show()
    print('#柱状图绘制完毕，保存png文件...')
    out_path = outputFile('.png')
    plt.savefig(out_path, dpi=300)
    print('=>保存完毕（{}）'.format(out_path))
    # 饼图
    plt.figure(figsize=(6,6))
    plt.pie(nums, labels=labels, autopct='%1.1f%%', explode=[0.1, 0.1, 0.1, 0.1])
    plt.title('PM2.5 离散化结果统计比例饼图')
    # plt.show()
...
def main():
    ...
    while True:
        ...
        print('3.读取txt文件按照列PM 2.5降序排序并导出到csv文件')
        print('4.读取csv文件对PM2.5进行离散化处理并绘制柱状图和饼图')
        print('0.退出')
        print('*************************************************************')
        num = input('>>> ')
        try:
